...
function PatientSignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/user/patient/signin', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Navigate to patient home page after 1 second
        setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex flex-col py-12 px-4 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="text-center text-gray-400">Sign in to access your health records</p>
      </div>
            ...
            <div className="relative">
              <div className="relative flex justify-center text-sm">
                <span className="px-2 text-gray-400">
                  ...
                  <Link to="/patient/signup" className="font-medium text-emerald-400 hover:text-emerald-300 transition-colors">
                    Sign Up
                  </Link>
                </span>
              </div>
